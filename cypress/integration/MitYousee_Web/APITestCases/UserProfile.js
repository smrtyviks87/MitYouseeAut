...
describe('UserProfile', function () {
    it('Verify Login Page', function () {
        ...
        cy.expectValidJsonWithMinimumLength('https://yss-api.internal.yousee.dk/yousee/selfcare/userprofile', 1, 'api_dsl')

        cy.LaunchApp(Url + "/qa/easylogin")
        cy.LoginApp('test', 'bcpp', 'api_coax', 'Test1234')

        cy.expectValidJsonWithMinimumLength('https://yss-api.internal.yousee.dk/yousee/selfcare/userprofile', 1, 'api_coax')

        Cypress.currentTest.retries(2) 

    })

    Cypress.Commands.add('expectValidJsonWithMinimumLength', (url, length,username) => {
        return cy.request({
            method: 'GET',
            url: url,
            ...
            .then((response) => {
                ...
                expect(body2).to.have.all.keys('userName', 'mobilePhoneNumber', 'emailAddress', 'userHash', 'ssoId');

                expect(body2).to.have.property('userName', username)
